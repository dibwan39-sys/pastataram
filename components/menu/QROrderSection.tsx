    ...
    <section className="section relative overflow-hidden" style={{ background: 'linear-gradient(180deg, var(--brand-surface) 0%, var(--brand-noir) 100%)' }}>
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <p className="text-sm md:text-base" style={{ color: 'rgba(216,194,189,0.7)' }}>
            {isAr ? 'امسح · تصفّح المنيو · اطلب من هاتفك مباشرة' : 'Scan · browse the menu · order right from your phone'}
          </p>
          <div className="w-10 h-0.5 rounded-full mx-auto mt-4" style={{ background: 'linear-gradient(90deg, #FD657D, #F0D3B0)' }} />
        </motion.div>
            ...
            {steps.map((s, i) => {
              ...
              return (
                <div key={s.title} className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-black text-base mb-0.5" style={{ color: '#FFF3EE' }}>{s.title}</h3>
                    <p className="text-sm leading-relaxed" style={{ color: 'rgba(216,194,189,0.7)' }}>{s.desc}</p>
                  </div>
                </div>
              )
            })}
