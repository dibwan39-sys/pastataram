...
/**
 * PROMOTIONS ARE CURRENTLY OFF.
 *
 * Every promotion in lib/data.ts expired in early 2025, and the page selected
 * them with `offers.filter(o => o.active)` — a flag that ignores the date — so
 * it kept presenting them as current. Their codes (DUO15, FAMILY20, HAPPY10)
 * were never read by the checkout either: nothing in the ordering flow has
 * ever applied a discount, so a customer who copied one had nowhere to use it.
 *
 * Rather than advertise promotions that do not exist, the route is hidden
 * while nothing is live. `liveOffers()` checks the flag AND the validity
 * window, so adding a genuine, in-date promotion to lib/data.ts brings the
 * page back on the next build with no code change here.
 *
 * Before any coupon is advertised again, checkout must actually validate and
 * apply it — see the implementation report.
 */
export default function OffersClient() {
  const { language } = useUIStore()
  const isAr = language === 'ar'

  const activeOffers = liveOffers()
  ...
  return (
    <PageWrapper>
      ...
      <section className="relative py-24 overflow-hidden bg-gradient-to-br from-brand-espresso to-brand-brown">
        <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(circle at 20% 50%, #E7C6A4 0%, transparent 50%)' }} />
          ...
          >
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <div className="text-center flex-shrink-0">
                ...
                {/* Promo code intentionally not rendered — see the note above. */}
              </div>
            </div>
          </motion.div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    {/* No promo code is shown. Checkout has no coupon field and
                        no validation rules, so displaying a code would promise
                        a discount the ordering flow cannot apply. */}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Link href="/menu" className="btn-primary text-xs px-4 py-2">
